...
import pandas as pd
import logging
from phone_price.utils import setup_logging, load_trained_model_and_preprocessor
...
def main():
    ...
    try:
        ...
        for i, (pred, proba) in enumerate(zip(predictions, probabilities if probabilities is not None else [None] * len(predictions))):
            category = price_categories.get(pred, "Неизвестно")
            print(f"Образец {i+1}: {category} (класс {pred})")
            if proba is not None:
                print(f"  Вероятности: {dict(zip(range(4), [f'{p:.3f}' for p in proba]))}")
